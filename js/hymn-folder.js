...
//   │   ├ 001.mp3
//   │   ├ 444.mp3
//   │   └ 444_pitch_-2_tempo_0_pitched.mp3   ← 음정 다른 것도 함께 두면 조절된다
//   └ 찬양/   … 함께 부르는 음원
//       └ 444.mp3
//
...
const HymnFolder = (() => {
  ...
  // ── 폴더 훑기 ────────────────────────────────────────────────────────
  // 결과: { mr: { "444": { "0": "444.mp3", "-2": "…" } }, song: {…} }
  async function scan() {
    const idx = { mr: {}, song: {} };
    const put = (r, e) => { (idx[r][e.num] = idx[r][e.num] || {})[e.pitch] = e.file; };
    ...
    if (cap()) {
      ...
      // 하위 폴더가 있으면 그쪽을, 없으면 뿌리의 mp3를 반주로 본다
      for (const [r, sub] of Object.entries(SUB)) {
        const files = await readdir(`${FOLDER}/${sub}`);
        files.forEach(f => { const e = parseName(f.name || f); if (e) put(r, e); });
      }
      rootFiles.forEach(f => {
        const nm = f.name || f;
        if (f.type === "directory") return;
        const e = parseName(nm); if (e && !idx.mr[e.num]) put("mr", e);
      });
    } else {
      ...
      for (const [r, sub] of Object.entries(SUB)) {
        const h = subs[sub]; if (!h) continue;
        for await (const [nm, fh] of h.entries()) {
          if (fh.kind !== "file") continue;
          const e = parseName(nm); if (e) put(r, e);
        }
      }
    }
    _save(K_IDX, idx);
  ...
  async function urlFor(num, r, pitch) {
    ...
    if (!bag) return null;
    const file = bag[String(pitch || 0)] ?? bag["0"] ?? bag[Object.keys(bag)[0]];
    if (!file) return null;
    const sub = SUB[r];

    if (cap()) {
      const FS = cap();
      // 하위 폴더에 있는지, 뿌리에 있는지 — 훑을 때와 같은 순서로 찾는다
      for (const path of [`${FOLDER}/${sub}/${file}`, `${FOLDER}/${file}`]) {
        try {
    ...
    const dir = await _dir(false);
    let fh = null;
    try { fh = await (await dir.getDirectoryHandle(sub)).getFileHandle(file); }
    catch (e) { try { fh = await dir.getFileHandle(file); } catch (e2) { return null; } }
  ...
  return {
    ...
    link, scan, index, clear, count, isLinked, have, urlFor, parseName
  };
})();
